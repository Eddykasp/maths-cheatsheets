...
Two sets are equal if they contain the same elements. If \(A\) and \(B\) are sets then their equality is expressed by \( \forall x(x\in A \leftrightarrow x\in B)\rightarrow A=B \).

\subsubsection{Venn Diagram}
The below diagram shows the set \(V\) contained in the universal set \(U\).
